      ...
      const formatedPools = await Promise.all(pools.map(async (pool) => {
        ...
        if (pool.kind === 'Snowglobe') {
          ...
          if (gauge) {
            ...
            const SNOBHarvestable = gauge.harvestable /1e18;
            const SNOBValue = SNOBHarvestable * data?.LastSnowballInfo?.snowballToken.pangolinPrice;
            return {
              address: pool.address, userLP, usdValue: userLP *
                pool.pricePoolToken, totalSupply, SNOBHarvestable, SNOBValue
            };
          }
        }
      }));

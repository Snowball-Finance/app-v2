...
export function S3fVaultContractProvider({ children }) {
  ...
  useEffect(() => {
    if (!!account && s3fContract && fraxContract && tusdContract && usdtContract && vaultContract && gaugeContract) {
      getInit();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [account, s3fContract, fraxContract, tusdContract, usdtContract, vaultContract, gaugeContract]);
  ...
  const getInit = async () => {
    try {
      ...
      const s3fBalanceValue = parseFloat(ethers.utils.formatUnits(s3fBalance, s3fToken.decimal))
      ...
      const s3fSupplyValue = parseFloat(ethers.utils.formatUnits(s3fSupply, s3fToken.decimal))

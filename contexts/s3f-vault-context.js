...
export function S3fVaultContractProvider({ children }) {
  ...
  const unsignedS3fContract = new ethers.Contract(
    CONTRACTS.S3F.TOKEN,
    ERC20_ABI,
    provider
  );
  const unsignedFraxContract = new ethers.Contract(
    CONTRACTS.S3F.FRAX,
    ERC20_ABI,
    provider
  );
  const unsignedTusdContract = new ethers.Contract(
    CONTRACTS.S3F.TUSD,
    ERC20_ABI,
    provider
  );
  const unsignedUsdtContract = new ethers.Contract(
    CONTRACTS.S3F.USDT,
    ERC20_ABI,
    provider
  );
  const unsignedVaultContract = new ethers.Contract(
    CONTRACTS.S3F.VAULT,
    S3F_VAULT_ABI,
    provider
  );
  ...
  const getSupply = async () => {
    try {
      const [s3fSupply, fraxSupply, tusdSupply, usdtSupply] = await Promise.all(
        [
  ...
  const getInit = async () => {
    try {
      ...
      const s3fPercentage = s3fSupplyValue
        ? parseFloat(s3fBalanceValue) / s3fSupplyValue
        : 0;

      setStaked(stakedValue);
      setSVToken((prev) => ({
        ...prev,
        balance: s3fBalanceValue,
        percentage: s3fPercentage,
        supply: s3fSupplyValue,
      }));
  ...
  const getDepositReview = async (data) => {
    ...
    const minToMint = await unsignedVaultContract.calculateTokenAmount(
      account,
      [fraxAmount, tusdAmount, usdtAmount],
      true
    );
  ...
  const removeLiquidity = async (
    ...
    try {
      ...
      const tokenAllowance = await s3fContract.allowance(
        account,
        CONTRACTS.S3F.VAULT
      );
      if (tokenAllowance.lt(calculatedWithdrawValue)) {
        const tokenApprove = await s3fContract.approve(
          CONTRACTS.S3F.VAULT,
          "10000000000000000000000000000000"
        );
        const transactionApprove = await tokenApprove.wait(1);

...
export function S3dVaultContractProvider({ children }) {
  ...
  const getInit = async () => {
    try {
      ...
      const s3dBalanceValue = parseFloat(ethers.utils.formatUnits(s3dBalance, s3dToken.decimal))
      ...
      const s3dSupplyValue = parseFloat(ethers.utils.formatUnits(s3dSupply, s3dToken.decimal))

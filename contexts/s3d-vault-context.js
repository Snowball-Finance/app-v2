...
export function S3dVaultContractProvider({ children }) {
  ...
  const unsignedS3dContract = new ethers.Contract(
    CONTRACTS.S3D.TOKEN,
    ERC20_ABI,
    provider
  );
  ...
  const unsignedBusdContract = new ethers.Contract(
    CONTRACTS.S3D.BUSD,
    ERC20_ABI,
    provider
  );
  const unsignedDaiContract = new ethers.Contract(
    CONTRACTS.S3D.DAI,
    ERC20_ABI,
    provider
  );
  ...
  const daiContract = useMemo(
    () =>
      library
        ? new ethers.Contract(CONTRACTS.S3D.DAI, ERC20_ABI, library.getSigner())
        : null,
    [library]
  );
  ...
  const getSupply = async () => {
    try {
      const [s3dSupply, usdtSupply, busdSupply, daiSupply] = await Promise.all([
      ...
      setBusdToken((prev) => ({
        ...prev,
        percentage: busdPercentage,
        supply: busdSupplyValue,
      }));
      setDaiToken((prev) => ({
        ...prev,
  ...
  const getInit = async () => {
    try {
      ...
      const s3dPercentage = s3dSupplyValue
        ? parseFloat(s3dBalanceValue) / s3dSupplyValue
        : 0;
      setStaked(stakedValue);
      setSVToken((prev) => ({
        ...prev,
        balance: s3dBalanceValue,
        percentage: s3dPercentage,
        supply: s3dSupplyValue,
      }));
  ...
  const getDepositReview = async (data) => {
    ...
    const minToMint = await unsignedVaultContract.calculateTokenAmount(
      account,
      [usdtAmount, busdAmount, daiAmount],
      true
    );
  ...
  const removeLiquidity = async (
    ...
    try {
      ...
      const tokenAllowance = await s3dContract.allowance(
        account,
        CONTRACTS.S3D.VAULT
      );
      if (tokenAllowance.lt(calculatedWithdrawValue)) {
        const tokenApprove = await s3dContract.approve(
          CONTRACTS.S3D.VAULT,
          "10000000000000000000000000000000"
        );
        const transactionApprove = await tokenApprove.wait(1);

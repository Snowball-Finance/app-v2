...
export function S4dVaultContractProvider({ children }) {
  ...
  const unsignedS4dContract = useMemo(
    () => new ethers.Contract(CONTRACTS.S4D.TOKEN, ERC20_ABI, provider),
    [provider]
  );
  const unsignedDaiContract = useMemo(
    () => new ethers.Contract(CONTRACTS.S4D.DAI, ERC20_ABI, provider),
    [provider]
  );
  const unsignedFraxContract = useMemo(
    () => new ethers.Contract(CONTRACTS.S4D.FRAX, ERC20_ABI, provider),
    [provider]
  );
  const unsignedTusdContract = useMemo(
    () => new ethers.Contract(CONTRACTS.S4D.TUSD, ERC20_ABI, provider),
    [provider]
  );
  const unsignedUsdtContract = useMemo(
    () => new ethers.Contract(CONTRACTS.S4D.USDT, ERC20_ABI, provider),
    [provider]
  );
  const unsignedVaultContract = useMemo(
    () => new ethers.Contract(CONTRACTS.S4D.VAULT, S4D_VAULT_ABI, provider),
    [provider]
  );
  ...
  const [svToken, setSVToken] = useState({
    name: "S4D",
    priceId: "s4d",
    decimal: 18,
    balance: 0,
  ...
  const [fraxToken, setFraxToken] = useState({
    ...tokenArray[1],
    balance: 0,
    supply: 0,
    percentage: 0,
  });
  const [tusdToken, setTusdToken] = useState({
    ...tokenArray[2],
    balance: 0,
    supply: 0,
    percentage: 0,
  ...
  const daiContract = useMemo(
    () =>
      library
        ? new ethers.Contract(CONTRACTS.S4D.DAI, ERC20_ABI, library.getSigner())
        : null,
    [library]
  );
  const fraxContract = useMemo(
    () =>
      library
        ? new ethers.Contract(
            CONTRACTS.S4D.FRAX,
  ...
  const getTokenContract = useCallback(
    ...
    [daiContract, fraxContract, tusdContract, usdtContract]
  );
  ...
  const getSupply = async () => {
    try {
      const [s4dSupply, daiSupply, fraxSupply, tusdSupply, usdtSupply] =
        await Promise.all([
      ...
      const totalSupply =
        daiSupplyValue + fraxSupplyValue + tusdSupplyValue + usdtSupplyValue;
      ...
      setSVToken((prev) => ({
        ...prev,
        supply: s4dSupplyValue,
        ratio: s4dRatio,
      }));
      setDaiToken((prev) => ({
        ...prev,
        percentage: daiPercentage,
        supply: daiSupplyValue,
      }));
      setFraxToken((prev) => ({
        ...prev,
        percentage: fraxPercentage,
        supply: fraxSupplyValue,
      }));
      setTusdToken((prev) => ({
        ...prev,
        percentage: tusdPercentage,
        supply: tusdSupplyValue,
      }));
      setUsdtToken((prev) => ({
        ...prev,
        percentage: usdtPercentage,
        supply: usdtSupplyValue,
      }));
    } catch (error) {
  ...
  const getInit = useCallback(async () => {
    try {
      ...
      const s4dPercentage = s4dSupplyValue
        ? parseFloat(s4dBalanceValue) / s4dSupplyValue
        : 0;
      setStaked(stakedValue);
      setSVToken((prev) => ({
        ...prev,
        balance: s4dBalanceValue,
        percentage: s4dPercentage,
        supply: s4dSupplyValue,
      }));
  ...
  const getTransactions = useCallback(async () => {
    try {
      ...
      while (events.length < 10 && attempt < 10) {
        const moreEvents = await unsignedVaultContract.queryFilter(
          "*",
          blockNumber - 500,
          blockNumber
        );
      ...
      await Promise.all(
        events.map(async (event) => {
          ...
          event.timestamp = getEnglishDateWithTime(
            ...
            for (let i = 0; i < 4; i++) {
              ...
              transactions = [
                ...transactions,
                {
                  ...
                  time: item.timestamp,
                  balance: -BNToString(
                    removeTokenAmounts[i],
                    removedToken.decimal
                  ),
                },
              ];
            }
  ...
  }, [getTokenById, provider, unsignedVaultContract]);

  const getToSwapAmount = useCallback(
    async (fromToken, toToken, fromAmount) => {
      try {
        if (fromAmount < 0 || !unsignedVaultContract) {
          return 0;
        }
        ...
        const toAmount = await unsignedVaultContract.calculateSwap(
          fromToken.index,
          toToken.index,
          fromAmount
        );
  ...
  const getWithdrawAmount = useCallback(
    async (withdrawPercentage, checkedValue) => {
      ...
      try {
        ...
        const calculatedWithdrawValue = calculatedWithdraw
          .mul(withdrawPercentage)
          .div(100);

        if (checkedValue === -1) {
          const removeAmounts =
            await unsignedVaultContract.calculateRemoveLiquidity(
              calculatedWithdrawValue
            );
          for (let i = 0; i < 4; i++) {
          ...
          const removeAmount =
            await unsignedVaultContract.calculateRemoveLiquidityOneToken(
              calculatedWithdrawValue,
              checkedValue
            );
    ...
    [setPopUp, getTokenById, unsignedVaultContract, account, svToken]
  );

  const getDepositReview = useCallback(
    async (data) => {
      if (!account) {
        setPopUp({
          title: "Network Error",
      ...
      try {
        ...
        const minToMint = await unsignedVaultContract.calculateTokenAmount(
          [daiAmount, fraxAmount, tusdAmount, usdtAmount],
          true
        );
    ...
    [setPopUp, svToken, unsignedVaultContract, account]
  );

  const onSwap = useCallback(
    async ({ fromToken, toToken, fromAmount, toAmount, maxSlippage }) => {
      if (!account) {
        setPopUp({
          title: "Network Error",
          text: MESSAGES.METAMASK_NOT_CONNECTED,
    ...
    [setPopUp, setLoading, getInit, account, getTokenContract, vaultContract]
  );

  const addLiquidity = useCallback(
    async (liquidityData, maxSlippage, receivingValue) => {
      if (!account) {
        setPopUp({
          title: "Network Error",
          text: MESSAGES.METAMASK_NOT_CONNECTED,
      ...
      try {
        ...
        const addLiquidity = await vaultContract.addLiquidity(
          [daiAmount, fraxAmount, tusdAmount, usdtAmount],
          minToMintAmount,
          deadline
        );
  ...
  const removeLiquidity = useCallback(
    async (liquidityData, withdrawPercentage, maxSlippage, selectedToken) => {
      if (!account) {
        setPopUp({
          title: "Network Error",
          text: MESSAGES.METAMASK_NOT_CONNECTED,
      ...
      try {
        ...
        const tokenAllowance = await s4dContract.allowance(
          account,
          CONTRACTS.S4D.VAULT
        );
        if (tokenAllowance.lt(calculatedWithdrawValue)) {
          const tokenApprove = await s4dContract.approve(
            CONTRACTS.S4D.VAULT,
            ethers.constants.MaxUint256
          );
          const transactionApprove = await tokenApprove.wait(1);
        ...
        if (selectedToken === -1) {
          ...
          for (let i = 0; i < 4; i++) {
            ...
            minToRemoveAmount[i] = floatToBN(
              value * maxSlippage,
              token.decimal
            );
          }

          const removeLiquidity = await vaultContract.removeLiquidity(
            calculatedWithdrawValue,
            minToRemoveAmount,
            deadline
          );
          ...
          const minToRemoveAmount = floatToBN(
            value * maxSlippage,
            token.decimal
          );

          const removeLiquidityOneToken =
            await vaultContract.removeLiquidityOneToken(
              calculatedWithdrawValue,
              selectedToken,
              minToRemoveAmount,
              deadline
